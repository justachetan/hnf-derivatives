...
def namespace2dict(config):
    ...
    for k in config:
        if isinstance(config[k], argparse.Namespace):
            config[k] = namespace2dict(config[k])
...
def load_imf(log_path=None, config_fpath=None, epoch=None,
    ...
    if load_dataloaders:
        ...
        loaders = {
            "train": trainer.get_dataloader("train"),
            "test": trainer.get_dataloader("test"),
        }
    else:
        loaders = None
...
def parse_hparams(hparam_lst):
    ...
    for i, hparam in enumerate(hparam_lst):
        ...
        print(k, v)
        out[k] = v
        out_str += "%s=%s_" % (k.split(".")[-1], v.replace("/", "-"))
    print(out)
    print(out_str)
    print("=" * 80)
    return out, out_str
...
def update_cfgdict_with_hparam(cfg, k, v, strict=True):
    k_path = k.split(".")
    cfg_curr = cfg
    v = str(v)
    for k_curr in k_path[:-1]:
        assert k_curr in cfg_curr, "%s not in %s" % (k_curr, cfg_curr)
        cfg_curr = cfg_curr[k_curr]
    k_final = k_path[-1]
    if strict:
        # assert hasattr(cfg_curr, k_final), \
        #     "Final: %s not in %s" % (k_final, cfg_curr)
        assert k_final in cfg_curr, \
            "Final: %s not in %s" % (k_final, cfg_curr)
        v_type = type(cfg_curr[k_final])
        print("Type: %s" % v_type)
        v_to_be_set = v_type(v)
    else:
        if str.isdecimal(v):
        ...
        v_to_be_set = v
    # setattr(cfg_curr, k_final, v_to_be_set)
    cfg_curr[k_final] = v_to_be_set


def update_cfgdict_hparam_lst(cfg, hparam_lst, strict=True):
    hparam_dict, hparam_str = parse_hparams(hparam_lst)
    for k, v in hparam_dict.items():
        update_cfgdict_with_hparam(cfg, k, v, strict=strict)
    return cfg, hparam_str
...
def flat_dict(data):
    if isinstance(data, dict):
        ...
        for k, v in data.items():
            flat_d = flat_dict(v)
            for k1, v1 in flat_d.items():
                k_combined = k if len(k1) == 0 else "%s.%s" % (k, k1)
                flat_lst_out[k_combined] = v1
        return flat_lst_out    
    else:
        return {"": data}